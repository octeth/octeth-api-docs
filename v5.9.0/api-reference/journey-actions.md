...
- If the current day is in `AllowedDays` and the current time is between `DontSendBefore` and `DontSendAfter`, the subscriber proceeds immediately.
- If the current day is allowed but the current time is before `DontSendBefore`, the subscriber is held until `DontSendBefore` on the same day.
- If the current day is allowed but the current time is past `DontSendAfter`, or the current day is not in `AllowedDays`, the subscriber is held until `DontSendBefore` on the next allowed day.
...
| Criteria         | The array of criteria. [Please refer to the criteria object below](/v5.8.3/api-reference/criteria-syntax).   |
