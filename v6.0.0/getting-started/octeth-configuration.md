...
33. **Bounce Webhook Authentication**
    ...
    - **Fresh installations enforce; upgrades are unchanged.** From v6.0.0 the shipped `_dockerfiles/examples/.oempro_env.example` is `true`, so a new installation is authenticated from day one, when no sender exists yet to break. The **code** default stays `false`, and an absent or empty value is treated as OFF, so an upgrade keeps whatever you already have: the upgrade's environment merge only appends keys that are absent, and this key has shipped since v5.9.3. That split is deliberate. Flipping the code default would make every bounce POST answer 401 on installations whose sender has not been reconfigured, and that failure is visible only at the sender, so bounce processing would stop quietly.
