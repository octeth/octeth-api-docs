...
3. Download the latest version (e.g., `oempro-rel-v6.0.0.zip`)
...
```bash
rsync -avz --progress -e "ssh -p 22" ~/Downloads/oempro-rel-v6.0.0.zip root@203.0.113.10:/opt/
```
...
```bash
scp -P 22 ~/Downloads/oempro-rel-v6.0.0.zip root@203.0.113.10:/opt/
```
...
ls -lh /opt/oempro-rel-v6.0.0.zip
```

You should see the file size and confirmation that the file exists in `/opt/`.

::: warning Important
Replace `203.0.113.10` with your actual server IP address and `oempro-rel-v6.0.0.zip` with your downloaded version filename.
:::
